...
class EcpayPayment extends \Opencart\System\Engine\Model
{
    ...
    public function __construct($registry)
    {
        ...
        $this->helper = new \Opencart\System\Library\EcpayPaymentHelper($registry);
    }
        ...
        if ($status) {
            ...
            if (is_array($payment_methods) && count($payment_methods) > 0) {
                ...
                if (0 >= $cart_total || $cart_total > 50000) {
                    unset($method_data['option']['ipassmoney']);
                }
    ...
    // Save AIO response
    public function saveResponse($order_id = 0, $feedback = [])
    {
        if (empty($order_id) === true) {
            return false;
        }
