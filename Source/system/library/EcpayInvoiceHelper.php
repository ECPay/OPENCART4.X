...
class EcpayInvoiceHelper extends ModuleHelper
{
    ...
    public function get_ecpay_invoice_api_info($action = '', $test_mode = false)
    {
        ...
        if ($test_mode) {
            ...
            $api_info['merchantId'] = $this->config->get($this->setting_prefix . 'mid');
            $api_info['hashKey']    = $this->config->get($this->setting_prefix . 'hashkey');
            $api_info['hashIv']     = $this->config->get($this->setting_prefix . 'hashiv');
